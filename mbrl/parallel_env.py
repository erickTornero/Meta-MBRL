...
class ParallelVrepEnv(object):
    ...
    def __init__(self, max_path_length:int, ports:list, envClass, reward_type, cripple_rotor_list:list):
        """
        ...
        self.crippled_rotor_list    =   cripple_rotor_list
        ...
        self.ps = [
            Process(target=self.worker, args=(work_remote, remote, max_path_length, idremote, seed, port, crippled_rotor)) 
            for work_remote, remote, idremote, seed, port, crippled_rotor in zip(self.work_remotes, self.remotes, itertools.count(), seeds, ports, self.crippled_rotor_list)
        ]
    ...
    def worker(self, remote, parent_remote, max_path_length, idremote, seed, port_, crippled_rotor):
        #print('idremote', idremote)
        env = self.envClass(port=port_, reward_type=self.reward_type, fault_rotor=crippled_rotor)
